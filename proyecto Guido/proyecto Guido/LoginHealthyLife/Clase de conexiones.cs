...
namespace LoginHealthyLife
{
    public class Clase_de_conexiones
    {
        ...
        public static int UsuariosRepetidos(string usuario, string contra)
        {
            ...
            MySqlCommand cmd = new MySqlCommand("SELECT id FROM usuarios WHERE nombre='" + usuario + "'", conexion);
        ...
        {
            int retorno = 0;
            MySqlCommand comando = new MySqlCommand(string.Format("Insert into usuarios (nombre, contraseña) values ('{0}','{1}')", usuario, contra), Clase_de_datos.ObtenerConexion());
